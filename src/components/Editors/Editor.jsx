...
const socket = io("https://cvrrcollabnotes.herokuapp.com/"); //instance of socketio client // localhost:5000
...
const Editor = (p) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://cvrrcollabnotes.herokuapp.com/groups/${groupIdDest}`).then(
      //localhost:5000
      (x) => {
        x.json().then((data) => {
          setValue([data]);
      ...
      onChange={(opts) => {
        ...
        const ops = editor.operations
          .filter((o) => {
            if (o) {
              return (
                o.type !== "set_selection" && o.type !== "set_value" && !o.data
              );
            }
            return false;
          })
